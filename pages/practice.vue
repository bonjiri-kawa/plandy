  ...
  <div>
    <v-app>
      ...
      <v-row class="ml-3" justify="start">
        ...
        <v-col cols="12" sm="12" md="4" lg="2" xl="2">
          <v-select :items="priceItems" label="検索範囲(Km)" dense></v-select>
        </v-col>
      ...
      <v-row>
        ...
        <v-col md="3" class="pl-0 pr-3">
          <draggable tag="ul">
            <li v-for="item in dateItems" :key="item.no" class="date-item-one">
      ...
      <v-row class="mt-5">
        ...
        <div class="pa-1">
          <v-btn @click="deleteAllLocalStrage"
            >ローカルストレージ全部削除</v-btn
          >
        </div>
        <div class="pa-1">
...
export default Vue.extend({
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        function (this: any, error: any) {
          ...
          // TODO 現在地が取得できなくても使用できるようにする new google.maps.DirectionsService()するようにする　
          alert("現在位置が取得できませんでした。許可しないと使用できない")
          //エラーの場合は東京駅周辺に移動
  ...
  methods: {
        ...
        placeService.nearbySearch(
          ...
          function (this: any, results: any, status: any) {
            ...
            if (status === google.maps.places.PlacesServiceStatus.OK) {
              ...
              console.log(
                "hamada photo",
                results[0].photos[0].getUrl({ maxWidth: 640 })
              );
    ...
    initMap() {
      ...
      this.DS.route(route, function (response: any, status: any) {
        if (status !== "OK") {
          window.alert("Directions request failed due to " + status);
          return;
        } else {
    ...
    addDateSpots(): void {
      ...
      let destination: any;
      if (
        typeof this.selectedPlace.dateSpot.position.lat === "function" &&
        typeof this.selectedPlace.dateSpot.position.lng === "function"
      ) {
        console.log("funciton??");
        destination = {
          lat: this.selectedPlace.dateSpot.position.lat(),
          lng: this.selectedPlace.dateSpot.position.lng(),
        };
      } else {
        destination = {
          lat: this.selectedPlace.dateSpot.position.lat,
      ...
      localStorage.setItem(
        "lastDestination",
        JSON.stringify(this.selectedPlace.dateSpot.title)
      );
      _this.DS.route(route, function (response: any, status: any) {
        if (status !== "OK") {
          window.alert("Directions request failed due to " + status);
          return;
        } else {
          console.log(
            "waypoints placeTitle",
            _this.selectedPlace.dateSpot.title
          );
          _this.waypoints.push({ location: destination });
          _this.waypointsName.push(_this.selectedPlace.dateSpot.title);

          _this.dateItems.push({
            time: "10:00",
            name: _this.selectedPlace.dateSpot.title,
          });
          ...
          for (let i = 0; i < _this.markers.length; i++) {
            if (
              typeof _this.markers[i].position.lat === "function" &&
              typeof _this.markers[i].position.lng === "function"
            ) {
              latBoolean = _this.markers[i].position.lat() === destination.lat;
              lngBoolean = _this.markers[i].position.lng() === destination.lng;
              if (latBoolean === true && lngBoolean === true) {
                _this.markers[i].icon.url =
                  _this.iconArrayNumber[_this.iconNum++];
    ...
    removeDateSpots(): void {
      ...
      let _this = this;
      // デートスポットから削除
      _this.dateItems = _this.dateItems.filter(
        (el) => el.name !== this.selectedPlace.dateSpot.title
      );
      ...
      _this.waypoints = _this.waypoints.filter(
        (el) =>
          el.location.lat !== destination.lat &&
          el.location.lng !== destination.lng
      );
      ...
      destination = {
        lat: _this.waypoints[_this.waypoints.length - 1].location.lat,
        lng: _this.waypoints[_this.waypoints.length - 1].location.lng,
      };
