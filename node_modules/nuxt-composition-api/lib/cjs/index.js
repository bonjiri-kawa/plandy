...
var normalize = _interopDefault(require('normalize-path'));
...
const globalNuxt = '<%= options.globalNuxt %>'.includes('options')
    ? '$nuxt'
    : '<%= options.globalNuxt %>';
const globalContext = '<%= options.globalContext %>'.includes('options')
    ? '__NUXT__'
    : '<%= options.globalContext %>';
const isFullStatic = '<%= options.isFullStatic %>'.includes('options')
    ? false
    : '<%= options.isFullStatic %>' === 'true';
...
const ssrRef = (value, key) => {
    ...
    const _ref = CompositionApi.ref(val);
    if (value instanceof Function)
    ...
    const proxy = CompositionApi.computed({
...
const shallowSsrRef = (value, key) => {
    ...
    if (process.client) {
        if (process.env.NODE_ENV === 'development' && ((_a = window[globalNuxt]) === null || _a === void 0 ? void 0 : _a.context.isHMR)) {
            return CompositionApi.shallowRef(getValue(value));
        }
        return CompositionApi.shallowRef((_d = (_c = (_b = window[globalContext]) === null || _b === void 0 ? void 0 : _b.ssrRefs) === null || _c === void 0 ? void 0 : _c[key]) !== null && _d !== void 0 ? _d : getValue(value));
    }
...
const getHeadOptions = (options) => {
    const _head = options.head instanceof Function
        ...
        : CompositionApi.reactive(options.head);
...
const useMeta = (init) => {
    ...
    assign(_head, init || {});
    const refs = CompositionApi.toRefs(_head);
    if (process.client)
        CompositionApi.watch(Object.values(refs), vm.$meta().refresh, { immediate: true });
    return refs;
};
...
const compositionApiModule = function () {
    ...
    this.nuxt.hook('generate:done', async (generator) => {
        ...
        fsExtra.readdirSync(staticPath).forEach(file => fsExtra.copyFileSync(path.join(staticPath, file), path.join(distPath, file)));
    });
    ...
    const { dst: entryDst } = this.addTemplate({
        ...
        options: {
            ...
            publicPath: normalize(path.join(((_c = this.options.router) === null || _c === void 0 ? void 0 : _c.base) || '', '/')),
            globalContext,
            globalNuxt,
        },
    });
